...
def main(custom_commands:"list[str]"=sys.argv):
    """
    ...
    if '__main__.py' in custom_commands[0]: # python -m pyminifier
    ...
    parser.add_option(
        "-o", "--outfile",
        dest="outfile",
        default=f"./minified/{custom_commands[-1]}",
    ...
    options, files = parser.parse_args(args=custom_commands)
    if not files:
        parser.print_help()
        sys.exit(2)
